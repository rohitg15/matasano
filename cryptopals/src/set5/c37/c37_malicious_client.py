...
class Client:
    ...
    g = 2
    k = 3
    def __init__(self, server_address, server_port, email, password = "SomeArbitraryPassword!"):
        ...
        print "DEBUG [*] : client's password %s" % (self.P)


    def send_dh_key(self, custom_dh_pubkey = None):
        """ generate dh keys and send public key to server """
        self.a = randint(2,Client.N - 1)
        if custom_dh_pubkey == None:
            self.A = CryptoMath.mod_exp(Client.g, self.a, Client.N)
        else:
            self.A = custom_dh_pubkey
    ...
    def generate_secret_key(self, is_key_zero = False):
...
if __name__ == "__main__":
    argc = len(sys.argv)
    if argc != 5:
        print "usage : %s server_ip_address server_port user_email multiple_of_N_for_pubkey" % (sys.argv[0])
        exit(-1)
    server_ip = sys.argv[1]
    server_port = int(sys.argv[2])
    user_email = sys.argv[3]

    # generate malicious dh public key
    custom_dh_key_multiple = int(sys.argv[4])
    custom_dh_pubkey = custom_dh_key_multiple * Client.N

    # break authentication using malicious dh public key
    c = Client(server_ip, server_port, user_email)

    # send a multiple of N as the client's public key
    c.send_dh_key(custom_dh_pubkey)
    c.receive_dh_key()
    c.compute_shared_pubkey()

    # sending a multiple of N as the client's public key results in the shared secret number S being 0 at the server
    # hence we zero the client's shared secret number S as well
    c.generate_secret_key(True)
